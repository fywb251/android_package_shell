...
if [ $count -lt 1 ] ;then
     ...
     if [ $? -eq 0 ] ;then
     	  ...
     	  echo '查找并替换引用文件的路径'
     	  find . -type f -name "*.java" |xargs perl -pi -e 's/com.foreveross.chameleon/com.csair.impc/g';
     	  ...
     	  echo '修改URL中的BASE_WS'
     	  find . -type f -name "URL.java"|xargs perl -pi -e 's/10.108.1.217:18860/192.168.11.18:18860/g';
     	  echo '修改BASE-WEB';
     	  find . -type f -name "URL.java"|xargs perl -pi -e 's/10.108.1.217:8080/192.168.11.18:8080/g';
     	  echo '修改MUC_BASE'
     	  find . -type f -name "URL.java"|xargs perl -pi -e 's/115.28.0.60:8081/192.168.11.18:9000/g';
            echo '修改登陆的接口';
     	  ...
     	  echo '查找并替换mina的配置文件'
     	  find . -type f -name "mina_client.properties"|xargs perl -pi -e 's/serverIp=10.108.1.217/serverIp=192.168.11.18/g';
     	  find . -type f -name "mina_client.properties"|xargs perl -pi -e 's/serverPort=18567/serverPort=18567/g';

     	  echo '查找并替换即时通讯的服务器地址'
     	  find . -type f -name "xmpp_client.properties"|xargs perl -pi -e 's/xmppHost=115.28.0.60/xmppHost=10.108.68.99/g';
     	  echo '查找并替换即时通讯的服务器的端口'
     	  find . -type f -name "xmpp_client.properties"|xargs perl -pi -e 's/xmppPort=5222/xmppPort=5222/g';
